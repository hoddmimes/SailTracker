...
public class Server
{
    ...
    private void execute() {
         ...
         * Note! when using jersey the the servel mapping is determined by the @Path in the class see com.hoddmimes.rest.TestService.java
         */

        try {
            System.out.println("appdir: " + new File(mAppDir).getAbsolutePath());
            Context context = mTomcatServer.addWebapp("/sailtracker", new File(mAppDir).getAbsolutePath());
